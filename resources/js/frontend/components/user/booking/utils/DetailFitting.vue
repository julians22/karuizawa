<script setup>
    ...
    const onPrint = () => {
        ...
        let urlTarget = `/print-semi-custom/`;

        const product_type = booking.value.product_type;

        if(product_type == 'App\\Models\\SemiCustomOuterProduct')
        {
            ...
            <div class="relative flex-1 md:m-auto max-md:mx-10 rounded-2xl md:max-w-lg lg:max-w-xl xl:max-w-3xl 2xl:max-w-4xl">
                <div class="flex max-md:flex-col bg-white shadow-2xl p-10 rounded-2xl w-full h-full overflow-hidden">
                    ...
                    <div class="w-full font-roboto" v-if="booking">
                        ...
                        <div class="font-bold text-lg text-center capitalize">Order number: {{ booking.product.order_number }}</div>

                        <div class="bg-primary-50 my-5 w-full h-[1px]"></div>

                        <div>
                            <div class="text-sm text-center capitalize">total payment</div>
                            <div class="font-josefin font-light text-xl text-center uppercase tracking-widest">{{ priceFormat(booking.price) }}</div>
                        </div>
